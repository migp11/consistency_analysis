...
class UmFinder:
    """Class for finding unconnected modules in genome-scale models.
        This class implements the computation for finding unconnected modules,
        by detecting blocked reactions and gap metabolites and creating a
        bipartite graph as described in [1]_
        ...
        cc_method : str, fva
            The algorithm used to find the blocked reactions
            model. Alternative methods is fastcc
        report : bool, True
            If True, the program runs in verbose mode
        copy_model: bool, False
            If True, a copy of the original model is created
        ...
        BMC Systems Biology, 7(1), 114. http://doi.org/10.1186/1752-0509-7-114
    """

    def __init__(self, model, cc_method='fva', report=True, copy_model=False):
        ...
        if report:
            ...
            print("Model: %s" % self.model.id)

            print("- Nº of reactions: %i" % len(self.model.reactions))
            print("- Nº of metabolites: %i" % len(self.model.metabolites))
